...
claim.attachments = attachments.map(att => ({
  name: att.file_name,
  url: `http://13.232.23.97:3031/uploads/${encodeURIComponent(att.file_path)}`,
  size: att.file_size
}));
    }
...
app.get('/api/claims/:id', async (req, res) => {
  ...
  try {
    ...
    claim.attachments = attachments.map(att => ({
      name: att.file_name,
      url: `http://13.232.23.97:3031/${att.file_path}`,
      size: att.file_size
    }));
...
app.post('/api/claims', upload.array('attachments'), async (req, res) => {
  ...
  try {
    ...
    claim.attachments = attachments.map(att => ({
      name: att.file_name,
      url: `http://13.232.23.97:3031/${path.basename(att.file_path)}`,
      size: att.file_size
    }));
